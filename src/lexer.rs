...
use crate::{err::TokenizeError, idioms::{self, ReaderResult, read_one_or_more}, number::{Digit, Number, Sign}, token::Token};
        ...
        match input[pos] {
            ...
            'u' => {
                ...
                if HI_SURROGATE_MIN <= a && a <= HI_SURROGATE_MAX {
                    let ((), new_pos) = idioms::read_string(input, pos, "\\u")?;
                    pos = new_pos;
                    let mut b: u32 = 0;
                    //pos += 1;
                    for _ in 0..4 {
